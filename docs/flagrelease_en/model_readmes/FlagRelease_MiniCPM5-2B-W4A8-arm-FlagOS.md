...
| --- | --- | ---: | ---: | ---: |
| W4A8 | Mac M5 Pro CPU | 1017.78 | 91.66 | 338.35 |
| W4A8 | CIX P1 | 271.17 | 23.25 | 86.21 |
...
After repository publication, or using authorized access, download the W4A8 G128 checkpoint. Run this same download step on either platform:

```bash
export MODEL_REPO="FlagRelease/MiniCPM5-2B-W4A8-arm-FlagOS"
export MODEL_DIR="$HOME/Models/MiniCPM5-2B-W4A8-G128-FlagOS"
...
- Model-body weights: symmetric INT4, group size 128, compressed-tensors `int-quantized` format.
...
Pull the candidate W4 environment by immutable digest and inspect its identity:

```bash
export  CIX_IMAGE_REF='harbor.baai.ac.cn/flagrelease-public/minicpm5-2b-w4a8-armv9-tree_3.7.2_git4db2332e-gems_0.0.post1.dev3404_g03ff66323-vllm_0.24.0_cpu-plugin_0.0.0-cx_none-python_3.11.2-torch_2.11.0_cpu-pcp_none-gpu_cpu-arc_arm64-driver_none:202609051345'
...
Create an idle container with the exact G128 repository directory from Shared Model Download mounted read-only. Use the same files as macOS; no HeadG32 directory or replacement `lm_head` is allowed:

```bash
export MODEL_DIR="$HOME/Models/MiniCPM5-2B-W4A8-G128-FlagOS"
export RUN_DIR=/data/minicpm5-runtime/w4
...
sudo docker run --detach \
  --name minicpm5-w4-runtime \
  --network host \
  --cpuset-cpus 0,1,6,7,8,9,10,11 \
  --entrypoint /bin/bash \
  -e MODEL=/models/minicpm5 \
  -e PORT=18040 \
  -e SERVED_MODEL_NAME=minicpm5-w4 \
  -e QUANT_MODE=w4 \
...
sudo docker inspect minicpm5-w4-runtime \
...
```bash
sudo docker exec --detach minicpm5-w4-runtime /bin/bash -lc \
  'exec /usr/local/bin/serve-minicpm5 > /artifacts/server.log 2>&1'

ready=0
for attempt in $(seq 1 120); do
  if curl --noproxy '*' --max-time 5 -fsS http://127.0.0.1:18040/v1/models; then
    ready=1
    break
  fi
  sleep 5
...
The entry point ultimately runs `vllm serve` with the model mounted at `/models/minicpm5`, served model name `minicpm5-w4`, port 18040, maximum model length 8192, maximum 2048 batched tokens, one sequence, the `uni` executor and compilation mode 3.
...
curl --noproxy '*' --fail-with-body --max-time 300 \
  http://127.0.0.1:18040/v1/chat/completions \
...
Verify non-empty output and strict W4 kernel coverage:
...
assert coverage["route_stats"]["prepared_g128_linears"] == 168
assert coverage["route_stats"]["prepared_g32_linears"] == 0
print("CIX G128 body checks PASS; output-head route review still required")
print(response["usage"], coverage["route_stats"])
PY
```

These checks require 168 G128 body linears and zero G32 linears. They are necessary but not sufficient for release acceptance: retain the model checksums, HTTP response, full coverage JSON and server log, and verify that the BF16-stored output head uses a supported strict execution route. The previous HeadG32 coverage assertion must not be reused. Only publish CIX performance after this checkpoint passes the full acceptance procedure.

#### 7. Stop the Container

Stop and remove the W4 container before starting another MiniCPM service on a 32 GiB CIX P1 host:

```bash
sudo docker stop -t 30 minicpm5-w4-runtime
sudo docker rm minicpm5-w4-runtime
```
...
4. For macOS, set the base URL to `http://127.0.0.1:8000/v1` and model name to `minicpm5-w4a8`. For CIX P1, use `http://127.0.0.1:18040/v1` and `minicpm5-w4`. Use `EMPTY` if an API key value is required.
...
- Both platforms must use the W4A8 G128 release checkpoint and SHA256 specified above. Do not publish a separate HeadG32 model as this release or reuse its performance figures.
