...
# Evaluation Results
## Benchmark Result
| Metrics      | GLM-5.3-Flash-Nvidia-Origin | GLM-5.3-Flash-Kunlunxin-FlagOS |
|--------------|--------------------------------|--------------------------------------|
...
```bash
docker pull harbor.baai.ac.cn/flagrelease-public/glm-5.3-flash-bf16-kunlunxin001-gems5.0.0-treenone-cx0.13.0-plugin0.2.0-vllm0.20.2-cp310-pt29-xrt513-x64-5.0.21.47:202608281449
```

### Download Open-source Model Weights
```bash
pip install modelscope
modelscope download --model FlagRelease/GLM-5.3-Flash-BF16-kunlunxin-FlagOS --local_dir /data/GLM-5.3-Flash
```
...
docker run -d \
  ...
  -v /data:/data:ro \
  --entrypoint /bin/bash \
  harbor.baai.ac.cn/flagrelease-public/glm-5.3-flash-bf16-kunlunxin001-gems5.0.0-treenone-cx0.13.0-plugin0.2.0-vllm0.20.2-cp310-pt29-xrt513-x64-5.0.21.47:202608281449 \
  -lc 'sleep infinity'
```
### Start the Server
```bash
...
docker exec -d glm53-flash-flagos-p800 /bin/bash -lc '
  ...
  export VLLM_FL_GLM5_PROVIDER=auto
  ...
  exec /root/miniconda/envs/python310_torch29_cuda/bin/vllm serve /data/GLM-5.3-Flash \
    --served-model-name glm5.3-flash-bf16 \
    --host 0.0.0.0 \
    --port 8000 \
    ...
    --language-model-only \
    --enforce-eager >>/var/log/glm53-flash-vllm.log 2>&1
'
```
...
curl --noproxy '*' -fsS http://127.0.0.1:8000/v1/chat/completions \
  ...
  -d '{"model":"glm5.3-flash-bf16","messages":[{"role":"user","content":"Compute 17 * 19. Return only the integer."}],"temperature":0,"max_tokens":32}'
